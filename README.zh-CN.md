...
5h 42% ▼3      ← 比预算少 3 个点（绿）
5h 1% ○        ← 窗口刚开始，暂不判定 pace
5h 42% ●       ← 在容差以内（5h ±5，7d ±3）
5h 0% ↺        ← 窗口已重置，等下一次数据
5h ↺           ← 当前没有活跃窗口，下一条消息会开启新窗口
5h 42% ● ⧗     ← 数据超过 30 分钟没更新
```
...
**从现在开始平均**（7d 行）。周二就用掉了 60%？平均线会一直显示 ▲30 直到周日，这个数字已经无法指导任何行动。
按下按钮后，已用的部分视为沉没，只把余量平均分到剩余时间：进度条上多出一条虚线刻度（检查点）和一条淡刻度
（原预算线的位置），预算 / 差值 / 预计、菜单栏标记和通知都切换到新的线。窗口重置时自动失效，也可以用「清除」提前取消。
